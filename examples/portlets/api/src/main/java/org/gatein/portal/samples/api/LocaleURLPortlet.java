...
/**
 * This portlet shows how to leverage the portlet URL <code>gtn:lang</code> property to set the language for render urls.
 *
...
public class LocaleURLPortlet extends GenericPortlet {
    ...
    public void serveResource(ResourceRequest req, ResourceResponse resp) throws PortletException, IOException {
        ...
        writer.print("<html>" + "<head>" + "<script type='text/javascript'>" + "function openLinkInParent(){"
                + "var e = document.getElementById(\"" + namespace + "\");" + "var url = e.options[e.selectedIndex].value;"
                + "window.open(url,'" + namespace + "_parent');" + "window.focus();" + "}\n");
    ...
    protected void doView(RenderRequest req, RenderResponse resp) throws PortletException, IOException {
        ...
        writer.print("<script type='text/javascript'>" + "var " + remoteWindowName + "; function " + namespace + "_popup(url){"
                ...
                + "', 'width=256,height=128,scrollable=yes')" + "}" + "onload = function() {" + "if (typeof "
                + remoteWindowName + " != 'undefined') " + "{" + remoteWindowName + ".location.reload(true);" + "}" + "}"
                + "</script>");

        writer.print("<p><a href='#' onclick=\"" + namespace + "_remote=" + namespace + "_popup('" + resource + "')\">"
