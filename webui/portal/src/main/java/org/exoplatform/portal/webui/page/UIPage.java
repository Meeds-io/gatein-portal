...
public class UIPage extends UIContainer {
  ...
  /**
   * Property settable in the portal'S configuration.properties file. See also
   * {@link #isFullPreview()}. {@link #getDefaultEditMode()}.
   */
  ...
  /**
   * Returns a locally cached value of {@value #FULL_PREVIEW_PROPERTY} property
  ...
  public static boolean isFullPreview() {
    if (fullPreview == null) {
      /*
       * Lazy initialization: For performance reasons, we have chosen to prefer
       * to ignore the potential concurrent updates on app startup to some kind
       * of locking. The concurrent updates should be harmless as they all
  ...
  public static boolean isFullPreviewInPageEditor() {
    if (!isFullPreview()) {
      ...
      return uiPortalApp.getEditMode() == EditMode.PREVIEW && uiPortalApp.getEditLevel() == EditLevel.EDIT_PAGE;
    }
  }
  ...
  public void switchToEditMode(String pageName, String pageSiteType, String pageSiteName) throws Exception {
    ...
    if (pageContext == null) {
      ...
      Page page = layoutService.getPage(pageKey.format());
