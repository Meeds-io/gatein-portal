...
(function($, base, common, msg) {
  eXo.webui.UICalendar = {
    ...
    renderCalendar : function() {
      ...
      table += '<a data-placement="right" rel="tooltip" onclick="eXo.webui.UICalendar.changeMonth(-1);" class="actionIcon pull-left" data-original-title="'
          + msg.getMessage("PreviousMonth")
          + '"><i class="uiIconMiniArrowLeft uiIconLightGray"></i></a>';
      table += '<span>' + this.months[this.currentDate.getMonth()] + ', '
          + this.currentDate.getFullYear() + '</span>';
      table += '<a data-placement="right" rel="tooltip" onclick="eXo.webui.UICalendar.changeMonth(1);" class="actionIcon pull-right" data-original-title="'
          + msg.getMessage("NextMonth")
      ...
      for (var i = 0; i < 7; i++) {
        if (i == (8 - this.firstDayOfWeek) % 7) {
          table += ' <td><font color="red">'
              + this.weekdays[(i + this.firstDayOfWeek - 1) % 7]
              + '</font></td>';
        } else {
      ...
      if (startDayOfWeek == 0)
        startDayOfWeek = 7;
      _pyear = (this.currentDate.getMonth() == 0) ? this.currentDate
          .getFullYear() - 1 : this.currentDate.getFullYear();
      _pmonth = (this.currentDate.getMonth() == 0) ? 11 : this.currentDate
          .getMonth() - 1;
      _pday = this.getDaysInMonth(_pyear, _pmonth)
          - ((startDayOfWeek + ((8 - this.firstDayOfWeek) % 7)) % 7) + 1;

      _nmonth = (this.currentDate.getMonth() == 11) ? 0 : this.currentDate
          .getMonth() + 1;
      _nyear = (this.currentDate.getMonth() == 11) ? this.currentDate
          .getFullYear() + 1 : this.currentDate.getFullYear();
      _nday = 1;
      ...
      for (var week = 0; week < 6; week++) {
        ...
        for (var dayOfWeek = 0; dayOfWeek <= 6; dayOfWeek++) {
          ...
          if (validDay) {
            ...
            tableRow = tableRow
                + '<td><a href="#SelectDate" class="otherMonth" onclick="eXo.webui.UICalendar.setDate('
                + _pyear + ',' + (_pmonth + 1) + ',' + _pday + ')">' + _pday
                + '</a></td>';
            _pday++;
          } else if (validDay == 0 && week != 0 && _weekend == week) {
            tableRow = tableRow
                + '<td><a href="#SelectDate" class="otherMonth" onclick="eXo.webui.UICalendar.setDate('
                + _nyear + ',' + (_nmonth + 1) + ',' + _nday + ')">' + _nday
                + '</a></td>';
            _nday++;
          }
        }
    ...
    getDaysInMonth : function(year, month) {
      return [ 31,
          ((!(year % 4) && ((year % 100) || !(year % 400))) ? 29 : 28), 31, 30,
