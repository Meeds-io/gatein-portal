...
/**
 * This class manages the drag and drop of components on the page. It uses a
 * DradDrop object to manage the events, sets some callback functions and some
 * parameters to initialize the DragDrop object.
 */
...
(function($, base, common) {
  eXo.portal.PortalDragDrop = {
    ...
    /**
     * This function inits the PortalDragDrop object It initializes a DragDrop
     * object that will manage the drag events
     */
    init : function(dragClasses) {
      ...
      $(".UIPageBody").find(".DragControlArea, .uiIconDragDrop").off(
      ...
      var initCallback = function(x, y, mouseX, mouseY, e) {
        if (eXo.portal.isInDragging)
          return;
        var dragObject = this, jDragObj = $(this);

        var origDragObjectStyle = {};
        var properties = [ "top", eXo.core.I18n.isLT() ? "left" : "right",
            "zIndex", "opacity", "filter", "position", "width" ];
        ...
        if (!isAddingNewly && browser.isIE7()) {
          originalDragObjectLeft = browser.findPosXInContainer(dragObject,
              $("#UIWorkingWorkspace")[0]);
        }

        // use this when press ESC with firefox (cancel dragdrop in column
        // container)
        ...
        if (isAddingNewly) {
          ...
          jDragObj.width(componentBlockWidth).find(".EDITION-BLOCK .NewLayer")
              .each(function() {
        ...
        var objAndMouse = browser
            .findMouseRelativeX(dragObject.offsetParent, e)
            - originalDragObjectLeft;
        ...
        if (eXo.core.I18n.isRT() && !isAddingNewly
            && !jDragObj.hasClass("UIPageBody")) {
          dragObject.style.left = originalDragObjectLeft
              + backupDragObjectWidth - componentBlockWidth + "px";
        } else {
          // rt and lt are the same for UIPageBody and newly added obj
          dragObject.style.left = dragObjectLeft + "px";
        }

        jDragObj.data("dropableTargets", PortalDragDrop
            .findDropableTargets(dragObject));
      ...
      var dragCallback = function(nx, ny, ex, ey, e) {
        ...
        var foundTarget = PortalDragDrop.findTarget(dragObject, ex, ey)
            || jDragObj.data("lastFoundTargetObject");
        if (!foundTarget)
          return;
        jDragObj.data("lastFoundTargetObject", jDragObj
            .data("foundTargetObject"));
        ...
        if (foundTarget.className == "UIPage") {
          ...
          if (eXo.portal.portalMode % 2)
            uiComponentLayout = jTarget.find(".LAYOUT-PORTAL").first();
          else
            uiComponentLayout = jTarget.find(".VIEW-PORTAL").first();
        } else {
          var foundUIComponent = $(foundTarget);
          if (eXo.portal.portalMode % 2)
            uiComponentLayout = foundUIComponent.find(".LAYOUT-BLOCK").first();
          else
            uiComponentLayout = foundUIComponent.find(".VIEW-BLOCK").first();
        ...
        layoutTypeElement = layoutTypeElement.length == 0 ? null
            : layoutTypeElement[0];
        ...
        if (layoutTypeElement != null
            && !$(layoutTypeElement).hasClass("UITableColumnContainer")) {
          /*
           * ===============================CASE ROW
           * LAYOUT================================
           */
          var rowContainer = jTarget.find(".UIRowContainer").first();
          var listComponent = rowContainer.children("div").filter(
              function() {
                return this.className != "DragAndDropPreview"
                    && this != dragObject;
              });

          jDragObj.data("listComponentInTarget", listComponent);
          var insertPosition = eXo.portal.PortalDragDrop.findInsertPosition(
              listComponent, "row", ey);
          if (jDragObj.data("foundTargetObject") === jDragObj
              .data("lastFoundTargetObject")
              && insertPosition === jDragObj.data("foundIndex"))
            return;
          jDragObj.data("foundIndex", insertPosition);

          /* Insert preview block */
          if (insertPosition >= 0) {
            rowContainer[0].insertBefore(previewBlock,
          ...
          var listComponent = trContainer.children("td").filter(
              function() {
                return this != previewBlock.parentNode
                    && this != dragObject.parentNode;
              });

          var insertPosition = eXo.portal.PortalDragDrop.findInsertPosition(
              listComponent, "column", ex);
          if (jDragObj.data("foundTargetObject") === jDragObj
              .data("lastFoundTargetObject")
              && insertPosition === jDragObj.data("foundIndex"))
            return;
          ...
          /* Insert preview block */
          if (insertPosition >= 0) {
            trContainer[0].insertBefore(dragObject.parentNode,
                listComponent[insertPosition]);
          } else {
        ...
        if (dragParent.children("div").length == 1
            && !dragParent.hasClass("EmptyContainer")) {
      ...
      var dropCallback = function(x, y, clientX, clientY, e) {
        ...
        if (!jDragObj.data("foundTargetObject")) {
          jDragObj.data("foundTargetObject", jDragObj
              .data("lastFoundTargetObject"));
        }
        ...
        // Case RowContainer : When dragobject is next to preview object
        // (position is not changed)
        // Case ColumnContainer : When dragObject.parent's lastSibling doesn't
        // change
        ...
        if (e.which !== 27) {
          ...
          if (dragObject.parentNode
              && dragObject.parentNode.tagName.toLowerCase() == "td") {
        ...
        if (previewBlock)
          previewBlock.parentNode.removeChild(previewBlock);
        previewBlock = null;
      ...
      $("." + dragClasses).each(
          function() {
            var dragBlock = this;
            var clickObject = $(this).find(".DragControlArea, .uiIconDragDrop")
                .last();
     ...
     * 
     * 1. Remove the dragging object if any 2. Send an request to server side to
     * update the changes
     */
    doDropCallback : function(dragObject) {
      ...
      if (!targetElement || foundIndex == undefined) {
        if (dragObject.isAddingNewly) {
          ...
          // Make IE7 rerender dom correctlly, this is a workaround for
          // EXOGTN-1092
      ...
      if (!srcElement.isAddingNewly && (foundIndex != undefined)) {
        if (PortalDragDrop.layoutTypeElementNode != null) {
          eXo.portal.PortalDragDrop.divRowContainerAddChild(srcElement,
      ...
      ajaxGet(eXo.env.server.createPortalURL("UIPortal", "MoveChild", true,
          params));
    },
    ...
    findDropableTargets : function(dragBlock) {
      ...
      if (uiRoot.length
          && eXo.portal.UIPortal.canMoveComponent(dragBlock, uiRoot[0])) {
    ...
    /**
     * Return a most suiable position among the <code>components</code>
     * objects that the dragging object should be at
     * 
     * @param layout
     *          {string} the layout type which is "row" or "column"
     */
    findInsertPosition : function(components, layout, mousePos) {
      ...
      if (layout == "row") {
        for (var i = 0; i < components.length; i++) {
          ...
          var componentMIddle = componentTop
              + Math.round(components[i].offsetHeight / 2);
          if (mouseYInPage > componentMIddle)
            continue;
          else
            return i;
        }
        ...
        for (var i = 0; i < components.length; i++) {
          ...
          var componentX = browser
              .findPosX(components[i], eXo.core.I18n.isRT());
          if (eXo.core.I18n.isRT()) {
            if (mouseXInPage < componentX)
              continue;
          } else if (mouseXInPage > componentX)
            continue;
    ...
    /**
     * Add the <code>srcElement</code> dragging object to a container. If the
     * dragging object is a column then let remove it from the table column
     * container
     */
    divRowContainerAddChild : function(srcElement, targetElement,
        insertPosition) {
      ...
      if (insertPosition >= 0) {
        uiRowContainer[0].insertBefore(srcElement,
            listComponent[insertPosition]);
      } else {
