...
(function($, base, common, uiMaskLayer, uiPopup) {
  var popupWindow = {
     ...
     * property of the elements in the page : . gets the z-index of the
     * maskLayer . gets all the other popup windows . gets the highest z-index
     * from these, if it's still at 0, set an arbitrary value of 2000 sets the
     * position of the popup on the page (top and left properties)
     */
    show : function(popupId, isShowMask, middleBrowser, height) {
      ...
      if ($(popup).find("iframe").length > 0) {
        setTimeout(function() {
          popupWindow.setupWindow(popup, middleBrowser, height);
        }, 500);
      } else {
    ...
    setupWindow : function(popup, middleBrowser, height) {
      ...
      if (browserHeight < jPopup[0].offsetHeight) {
        vrez.css("max-height", "");
        var vrezHeight = browserHeight - jPopup[0].offsetHeight + vrez.height()
            - 20;
      ...
      if (offsetParent) {
        ...
        if (middleBrowser || !middleWindow) {
          popup.style.top = Math.ceil((browserHeight - popup.offsetHeight) / 2)
              + scrollY + "px";
        }
        // Todo: set popup of UIPopup always display in the center browsers in
        // case UIMaskWorkspace
    ...
    hide : function(popupId, isShowMask) {
      ...
      if (isShowMask)
        popupWindow.showMask(popup, false);
    },
     ...
     * onmouseup events on the portal application (not the popup) associates
     * these events with UIPopupWindow.resize and UIPopupWindow.endResizeEvt
     * respectively
     */
    startResizeEvt : function(evt) {
      ...
      popupWindow.backupPointerY = base.Browser.findMouseRelativeY(targetPopup,
          evt);

      var jDoc = $(document);
      jDoc.on("mousemove.UIPopupWindow touchmove", popupWindow.resize);
      jDoc.on("mouseup.UIPopupWindow touchend", popupWindow.endResizeEvt);
    },

    /**
     * Function called when the window is being resized . gets the position of
     * the mouse . calculates the height and the width of the window from this
    ...
    /**
     * Called when the window stops being resized cancels the mouse events on
     * the portal app inits the scroll managers active on this page (in case
     * there is one in the popup)
     */
    endResizeEvt : function(evt) {
      popupWindow.resizedPopup = null;
      popupWindow.vresized = null;
      $(document).off("mousemove.UIPopupWindow touchmove").off("mouseup.UIPopupWindow touchstop");

      // enable select text
      if (navigator.userAgent.indexOf("MSIE") >= 0) {  
        document.onselectstart = popupWindow.backupEvent;
      } else {
        $(document).off("mousedown touchstart").on("mousedown touchstart",
            popupWindow.backupEvent);
      }
      popupWindow.backupEvent = null;
    },
    ...
    initDND : function(popupBar, popup) {
      ...
      popup.onDragStart = function(x, y, last_x, last_y, e) {
        if (base.Browser.isFF() && popup.uiWindowContent) {
          ...
          $(popup.uiWindowContent).find("ul.PopupMessageBox").css("overflow",
              "auto");
        }
      };
    ...
    getResizeBlock : function(jPopup) {
      ...
      jPopup.find(".resizable .resizable").filter(filterPopup).removeClass(
          "resizable");
      var innerRez = jPopup.find(".resizable").filter(filterPopup);
      var contentBlock = jPopup.find("div.PopupContent, .popupContent").filter(
          filterPopup);
