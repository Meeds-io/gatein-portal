...
(function($, base, uiPopup) {
  eXo.core.UIMaskLayer = {
    ...
    createTransparentMask : function(position) {
      ...
      browser.addOnScrollCallback("5439383",
          eXo.core.UIMaskLayer.scrollCallback);
      ajaxLoading.hide();
     ...
     * the opacity in % . the position between : TOP-LEFT, TOP-RIGHT,
     * BOTTOM-LEFT, BOTTOM-RIGHT, other value will position to center The
     * returned element has the following html attributes : . className =
     * "MaskLayer" ; . id = "MaskLayer" ; . style.display = "block" ; .
     * maxZIndex = 2 ; . style.zIndex = maskLayer.maxZIndex ; . style.top =
     * "0px" ; . style.left = "0px" ;
     */
    createMask : function(blockContainerId, object, opacity, position) {
      try {
        ...
        if (offsetParent.length && offset.left != 0 && offsetParent.top != 0) {
        ...
        if (object != null) {
          ...
          if (eXo.core.I18n.isLT()) {
            if ((blockContainer[0].offsetWidth > object.offsetLeft
              ...
              object.style.left = blockContainer[0].offsetWidth
    ...
    createMaskForFrame : function(blockContainerId, object, opacity) {
      try {
        ...
        window.setTimeout(function() {
          ...
          maskLayer.css("top", browser.findPosYInContainer(blockContainer[0],
              parentOfBlockContainer)
              + "px");
          maskLayer.css("left", browser.findPosXInContainer(blockContainer[0],
              parentOfBlockContainer)
              + "px");
        }, 200);
    ...
    /**
     * Set the position of the mask layer, depending on the position attribute
     * of UIMaskLayer position is between : TOP-LEFT, TOP-RIGHT, BOTTOM-LEFT,
    ...
    removeMask : function(maskLayer) {
      if (maskLayer) {
        ...
        nextSiblingOfObject.parentNode.insertBefore(maskLayer.nextSibling,
            nextSiblingOfObject);
        nextSiblingOfObject.parentNode.removeChild(nextSiblingOfObject);
