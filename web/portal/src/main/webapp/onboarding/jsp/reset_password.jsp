...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  <body>
  	...
  	<p id="WelcomeContent"><%=res.getString("onboarding.login.welcomeTo")%>  <%=logoTitle%>!</p>
    <div class="uiLogin">
     <div class="uiLoginCondition">
      ...
      <p><%=res.getString("onboarding.login.allowCreatePassword")%></p>
        ...
        <div class="centerLoginContent">
          <form name="registerForm" action="<%= contextPath + forgotPasswordPath %>" method="post" style="margin: 0px;">
                <% if (uri != null) {
                ...
                <div class="userCredentials">
                  ...
                  <input tabindex="1" id="username" name="username" type="text" data-validation="require"  readonly="readonly"  name="username" value="<%=username%>" >
                </div>
                <div class="userCredentials">
                  <span class="iconPswrd"></span>
                  <input  tabindex="2"  data-validation="require" type="password" id="password" name="password" value="<%=(password != null ? password : "")%>"  placeholder="<%=res.getString("portal.login.Password")%>" onblur="this.placeholder = '<%=res.getString("portal.login.Password")%>'" onfocus="this.placeholder = ''">
                </div>
                <div class="userCredentials">
                  <span class="iconPswrd"></span>
                   <input  tabindex="2"  data-validation="require" type="password" id="password2" name="password2" value="<%=(password2 != null ? password2 : "")%>"  placeholder="<%=res.getString("onboarding.login.ConfirmPassword")%>" onblur="this.placeholder = '<%=res.getString("onboarding.login.ConfirmPassword")%>'" onfocus="this.placeholder = ''">
                </div>
                <p class="capatchaMessage"><%=res.getString("onboarding.login.capatcha")%></p>
				<div id="UIPortalLoginFormAction" class="loginButton">
					<button class="button" type="submit" tabindex="4"><%=res.getString("onboarding.login.create")%></button>
				</div>
