    ...
    if (token != null && !token.isEmpty()) {
      ...
      if ("saveExternal".equalsIgnoreCase(requestAction)) {
        ...
        if (reqFirstName == null || reqLastName == null) {
          ...
          if (password == null || !customPasswordPattern.matcher(password).matches()
              || customPasswordMaxlength < password.length() || customPasswordMinlength > password.length()) {
            String passwordpolicyProperty = PropertyManager.getProperty("gatein.validators.passwordpolicy.format.message");
            errors.add(passwordpolicyProperty != null ? passwordpolicyProperty
                                                      : bundle.getString("onboarding.login.passwordCondition"));
          }
        ...
        if (errors.isEmpty()) {
          ...
          try {
            organizationService.getUserHandler().createUser(user, true);// Broadcast
                                                                        // user
                                                                        // creation
                                                                        // event
