...
public class BrandingServiceImpl implements BrandingService, Startable {
  ...
  public void updateBrandingInformation(Branding branding) {
    try {
      ...
      if (branding.getLogo() != null && StringUtils.isNotBlank(branding.getLogo().getUploadId())) {
        updateLogo(branding.getLogo(), false);
      }
      if (branding.getFavicon() != null && StringUtils.isNotBlank(branding.getFavicon().getUploadId())) {
  ...
  private void updateLogo(Logo logo, boolean updateLastUpdatedTime) {
    ...
    if (logo == null || ((logo.getData() == null || logo.getData().length <= 0) && StringUtils.isBlank(logo.getUploadId()))) {
      ...
      try {
        ...
        } else if (StringUtils.isNotBlank(logo.getUploadId())) {
  ...
  private void updateFavicon(Favicon favicon, boolean updateLastUpdatedTime) {
    ...
    if (favicon == null || ((favicon.getData() == null || favicon.getData().length <= 0) && StringUtils.isBlank(favicon.getUploadId()))) {
      ...
      try {
        ...
        } else if (StringUtils.isNotBlank(favicon.getUploadId())) {
